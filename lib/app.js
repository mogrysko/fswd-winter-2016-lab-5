...
app.param('todo_id', function(request, response, next, id) {
...
app.get('/todo/:todo_id', function(request, response) {
  response.format({
    'text/html': function() {
      response.render('todo', { todo: request.todo });
    },
    'application/json': function() {
      response.json(todo);
    }
...
app.post('/todo/:todo_id/complete', function(request, response) {
  request.todo.markCompleted().then(function(){
    response.redirect('/todo/' + request.todo.id)
  })
});
